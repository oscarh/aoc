...
pub fn run() {
    ...
    println!(
        "Part 1 (example data): {}",
        part1(util::lines_iter(&mut part_1_example))
    );
    println!(
        "Part 1: {}",
        part1(util::lines_iter(File::open("input/input_03.txt").unwrap()))
    );
    let mut part_2_example =
        &b"xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))"[..];
    println!(
        "Part 2 (example data): {}",
        part2(util::lines_iter(&mut part_2_example))
    );
    println!(
        "Part 2: {}",
        part2(util::lines_iter(File::open("input/input_03.txt").unwrap()))
    );
}
